...
class Book{
    ...
    Book(string t,float p,int s):bookID(bcount++){
        title = new string(t);
        price = new float(p);
        stock=new int(s);
        cout<<"changes saved\n";
    }
    Book(const Book &b):bookID(bcount++) {
    ...
    void display(){
        cout<<"book title: "<<*title<<" Book id: "<<bookID<<" price: "<<*price<<" stock: "<<*stock<<endl;
    }
};
int Book:: bcount=0;
int main(){
    Book b1("matilda",49.9,20);
    Book b2("harry potter",60,30);
    b1.purchase(11);
    b1.display();
    b2.display();
}
